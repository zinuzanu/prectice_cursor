...
- **정렬/페이징**: Spring Data `Pageable` 기반 목록 페이징/정렬
- **검색 기능**: 제목/내용 키워드 검색(`q`)
- **검증 UX 강화**: 필드별 제약 + 커스텀 한국어 메시지(`messages.properties`)
- **XSS/HTML 처리 정책**: `content`는 기본적으로 HTML escape되어 출력(안전 우선)
- **테스트 보강**: Repository(@DataJpaTest), API(MockMvc) 테스트
- **운영 DB 전환(템플릿)**: `application-dev/mysql/postgres.properties` 추가(드라이버/실 DB는 별도)
- **REST API 확장**: `/api/posts` CRUD 제공

### 다음으로 구현하기 좋은 아이디어(추천)

- **정렬 UI 개선**: 목록에서 `sort`/방향(asc/desc)을 버튼/드롭다운으로 선택 가능하게
- **페이지네이션 UX**: 페이지가 많을 때 `1 ... 5 6 7 ... N` 형태로 “윈도우” 표시
- **보안/인증**: Spring Security로 로그인(작성/수정/삭제 권한), CSRF 정책 정리
- **댓글/좋아요**: 게시글 연관 엔티티로 댓글/리액션 기능 추가
- **파일 업로드**: 이미지/첨부파일 업로드 + 다운로드(로컬/클라우드 스토리지)
- **에디터 정책**: Markdown 렌더링 도입(escape vs sanitize 정책 포함) 또는 WYSIWYG 편집기
- **API 문서화**: Springdoc OpenAPI(Swagger UI)로 `/api` 문서 자동화
- **마이그레이션**: Flyway/Liquibase로 스키마 변경 이력 관리
...
- [x] IDE 스타일 레이아웃 적용 (Topbar/Sidebar/Tabbar)
- [x] 기본 다크 테마 + Theme 토글
- [x] textarea 자동 확장, 글자수 카운터
- [ ] 목록 UI 추가 개선 (정렬 드롭다운, 검색 결과 강조, 빈 상태/로딩 상태)
- [ ] 상세/작성/수정 UX 추가 개선 (키보드 포커스 흐름, 단축키 등)
...
-- **YYYY-MM-DD**: 개선 작업(페이징/검색/검증 UX/REST API/테스트/프로필 분리)  
  - PR: `#(링크/번호)` / Branch: `feat/improvements` / Commit: `(merge commit)`
-- **YYYY-MM-DD**: IDE 스타일 UI 적용(레이아웃/테마/폼 UX 개선)  
  - PR: `#(링크/번호)` / Branch: `feat/ui-ide` / Commit: `(merge commit)`
